...
void SignalHandler(int /*signum*/) { g_shutdown.store(true); }
...
int run(int argc, char** argv) {
  ...
  bool control_manager_enabled = false;
  bool powered_on_here = false;
  bool servo_on_here = false;
  bool tool_flange_12v_on = false;
  bool master_port_open = false;
  bool gripper_port_open = false;
  bool cleanup_done = false;

  std::unique_ptr<RobotCommandStreamHandler<T>> stream;
  dynamixel::PortHandler* portHandler = nullptr;
  dynamixel::PacketHandler* packetHandler = nullptr;
  std::vector<int> activeIDs;
  dynamixel::PortHandler* portHandler_gripper = nullptr;
  dynamixel::PacketHandler* packetHandler_gripper = nullptr;
  std::vector<int> activeIDs_gripper;
  std::thread master_arm_handler;
  std::thread gripper_handler;
  ...
  auto cleanup = [&]() {
    ...
    if (gripper_handler.joinable()) {
      gripper_handler.join();
    }

    if (master_port_open && portHandler != nullptr && packetHandler != nullptr) {
      BulkWriteTorqueEnable(portHandler, packetHandler, activeIDs, TORQUE_DISABLE);
      portHandler->closePort();
      master_port_open = false;
    }

    if (gripper_port_open && portHandler_gripper != nullptr && packetHandler_gripper != nullptr) {
      for (int id : activeIDs_gripper) {
        TorqueEnable(portHandler_gripper, packetHandler_gripper, id, TORQUE_DISABLE);
      }
      portHandler_gripper->closePort();
      gripper_port_open = false;
    }

    if (control_manager_enabled) {
      robot->DisableControlManager();
      control_manager_enabled = false;
    }

    if (servo_on_here) {
      robot->ServoOff(servo);
      servo_on_here = false;
    }

    if (tool_flange_12v_on) {
      robot->SetToolFlangeOutputVoltage("right", 0);
      robot->SetToolFlangeOutputVoltage("left", 0);
      tool_flange_12v_on = false;
    }

    if (powered_on_here) {
      robot->PowerOff(kAll);
      powered_on_here = false;
    }
  };
  ...
  portHandler = dynamixel::PortHandler::getPortHandler(devicename_master_arm);
  ...
  portHandler_gripper = dynamixel::PortHandler::getPortHandler(devicename_gripper);
  packetHandler_gripper = dynamixel::PacketHandler::getPacketHandler(PROTOCOL_VERSION);
  ...
  master_arm_handler = std::thread(control_loop_for_master_arm, portHandler, packetHandler, activeIDs);
  gripper_handler = std::thread(control_loop_for_gripper, portHandler_gripper, packetHandler_gripper,
                                activeIDs_gripper);
