...
const std::string kAll = "^((?!wheel|torso|left|head).)*";
...
int run(int argc, char** argv) {
  ...
  q_joint_target = robot->GetState().position.block(2 + 6, 0, kDOF, 1) +
  ...
  q_joint = robot->GetState().position.block(2 + 6, 0, kDOF, 1);
