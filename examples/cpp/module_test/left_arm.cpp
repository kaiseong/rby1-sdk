...
const std::string kAll = "^((?!wheel|right|torso|head).)*";
...
int run(int argc, char** argv) {
  ...
  q_joint_target = robot->GetState().position.block(2 + 6 + 7, 0, kDOF, 1) +
  ...
  q_joint = robot->GetState().position.block(2 + 6 + 7, 0, kDOF, 1);
